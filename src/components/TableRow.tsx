...
export default function TableRow({id, kind, authors, title, publishedDate, index}) {
    ...
    return (
        ...
        >
            ...
            <td className='px-4'>{kind ? kind.map(kind => (
                <p className='text-start w-fit max-w-[350px]'>{kind}</p>)) : "N/A"}</td>
            <td className='px-4 py-2'>{authors ? authors.map(author => (
            ...
            <td className='px-4 max-w-[350px]'>
                <p onClick={() => bookDetailsButton(id)}
                   className='cursor-pointer text-start w-fit hover:underline'>{title ? title : "N/A"}</p>
            </td>
            <td className='px-4'>
                <p className='text-end'>{publishedDate ? publishedDate : "N/A"}</p>
            </td>
        </tr>
    )
}